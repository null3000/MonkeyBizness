AFRAME.registerComponent("pooltable", {
			init: function () {
				...
				cue.addEventListener("animationcomplete", function () {
					...
					setTimeout(function(){
	AllBall.body.velocity.set(0, 0, 0);
			...
			tick: function () {
				...
				for (var i = 0; i < this.objects.length; i++) {
					...
					if (ballY < .1) {
						ball.body.velocity.set(0, 0, 0);
						setTimeout(function(){
   ball.body.position.set(0, .25, -3);}, 5000);
					}
				

				}
				this.redscore.setAttribute("value", scorered);
				this.bluescore.setAttribute("value", scoreblue);
